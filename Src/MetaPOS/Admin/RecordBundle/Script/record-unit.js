...
function eventChange() {
    ...
    checkPagePermission("UnitMeasurement");

    displayDataTable();

    $('#txtName,#txtRatio').keypress(function (event) {
...
function displayDataTable() {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            var actionEditHtml = "<a id='btnUpdateModal' href='javascript:void(0)' class='glyIconPosition' data-toggle='modal' data-backdrop='static'>" +
                "<span class='glyphicon glyphicon-pencil'>" +
                "</span></a>";

            var actionHtml;
            if ($("#ddlActiveStatus").val() === "1") {
                actionHtml = actionEditHtml + "<a id='btnDeleteModal' href='javascript:void(0)' class='glyIconPosition' data-toggle='modal' data-backdrop='static'>" +
                    "<span class='glyphicon glyphicon-trash'>" +
                    "</span></a>";
            }
            else {
                actionHtml = "<a id='btnRestoreModal' href='javascript:void(0)' class='glyIconPosition' data-toggle='modal' data-backdrop='static'>" +
                   ...
                   "buttons": [
                       {
                           ...
                           "customize": function (win) {
                               ...
                               $(win.document.body).prepend('<p style="border-bottom: 1px solid #ccc; padding-bottom: 10px; padding-top: 3px;"><b>Unit List</b></p>');
...
$(document).on("click", "#btnSaveModal", function () {
    resetFormValue();
    $("#formModal .modal-title").html("<i class='fa fa-file-text' aria-hidden='true'></i>&nbsp;<span class='lang-record-add-unit'>" + Add_unit + "<span>");
    $("#btnUpdate").hide();
    $("#btnSave").show();
...
$(document).on("click", "#btnUpdateModal", function () {
    resetFormValue();
    $("#formModal .modal-title").html("<i class='fa fa-file-text' aria-hidden='true'></i>&nbsp;" + Edit_unit + "");
    $("#btnSave").hide();
    $("#btnUpdate").show();
    ...
    $('#txtRatio').val($(this).parent().parent().find(".unitRatio").html());
...
$(document).on("click", "#btnRestoreModal", function () {
    var id = $(this).parent().parent().find(".id").html();
...
function validateForm() {
    ...
    var unitName = $('#txtName').val();
    var unitRatio = $('#txtRatio').val();

    if (!unitRatio.match(/\d+/)) {
        validate = false;
        showModalOutput("Invalid Ratio number!", "warning");
    }
