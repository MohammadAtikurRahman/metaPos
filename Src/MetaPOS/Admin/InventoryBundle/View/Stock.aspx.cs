...
namespace MetaPOS.Admin.InventoryBundle.View
{
    ...
    public partial class Stock : BasePage
    {
        ...
        private void LoadStockDropdownlist()
        {
            ...
            ddlSupplierList.Items.Insert(0, new ListItem(Resources.Language.Lbl_stock_search_all_supplier, "0"));
            ...
            ddlCatagoryList.Items.Insert(0, new ListItem(Resources.Language.Lbl_stock_search_all_category, "0"));
            ...
            {
                ddlWarehouseList.Items.Insert(1, new ListItem("All Store", "0"));
            }
            ...
            ddlLocationList.Items.Insert(0, new ListItem(Resources.Language.Lbl_stock_search_all_location, "0"));
        ...
        private string GetStockSearchQuery(string productId)
        {
            ...
            {
                allStoreQty = "SUM(CAST(MIN(qtyManagement.stockQty as decimal))),0";
            }
            else
            {
                allStoreQty = "MIN(qtyManagement.stockQty),0";
                storeIdQuery = "AND qtyManagement.storeId = '" + ddlWarehouseList.SelectedValue + "'";
            }
            ...
            queryGrdDisplay = @"SET QUERY_GOVERNOR_COST_LIMIT 0;
                                ...
                                LEFT JOIN QtyManagement as qtyManagement ON qtyManagement.productId = stock.prodId " + storeIdQuery +" " +
        ...
        private void RefreshGrd(string queryGrid, bool isSearch)
        {
           ...
           // damageTotalQty = 0;
            ...
            //{
            //    var qty = Convert.ToDecimal(dtTotalStock.Rows[k]["qty"].ToString() == "" ? "0" : dtTotalStock.Rows[k]["qty"].ToString());
            //    var bPrice = Convert.ToDecimal(dtTotalStock.Rows[k]["bPrice"].ToString() == "" ? "0" : dtTotalStock.Rows[k]["bPrice"].ToString());
            //    var sPrice = Convert.ToDecimal(dtTotalStock.Rows[k]["sPrice"].ToString() == "" ? "0" : dtTotalStock.Rows[k]["sPrice"].ToString());
            //    var wholeSale = Convert.ToDecimal(dtTotalStock.Rows[k]["dealerPrice"].ToString() == "" ? "0" : dtTotalStock.Rows[k]["dealerPrice"].ToString());
        ...
        protected void grdStock_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            ...
            {
                var attributeValue = ((Label)e.Row.FindControl("lblAttributeValue")).Text;
                if (attributeValue != "0")
                {
                    string AttributeLabel = "";
                    string[] splitAttrVal = attributeValue.Split(',');
                    for (int i = 0; i < splitAttrVal.Length; i++)
                    {
                        string attrVal = splitAttrVal[i].ToString();
                        if (String.IsNullOrWhiteSpace(attrVal))
                            break;
                        var dtAttribute = sqlOperation.getDataTable("SELECT attributeName FROM AttributeInfo WHERE Id='" + attrVal + "'");
                        if (dtAttribute.Rows.Count > 0)
                            AttributeLabel += dtAttribute.Rows[0]["AttributeName"].ToString() + ",";
                    }

                    AttributeLabel = "(" + AttributeLabel.Substring(0, AttributeLabel.Length - 1) + ")";

                    ((Label)e.Row.FindControl("lblAttributeLabel")).Text = AttributeLabel;
                }
            ...
            var WarehouseId= ddlWarehouseList.SelectedValue;
            if (e.Row.RowType == DataControlRowType.Footer)
            {
                if (WarehouseId == "0")
                {
                    string stockQuery = "select SUM(CAST(stockQty as decimal)) as totalQty from QtyManagement";
                    var dtStockQty = sqlOperation.getDataTable(stockQuery);
                    var totalQty = dtStockQty.Rows[0]["totalQty"].ToString() == "" ? "0" : dtStockQty.Rows[0]["totalQty"].ToString();
                    var stockQty = Convert.ToDecimal(totalQty);

                    Label lbl = (Label)e.Row.FindControl("lblQtyFooter");
                    lbl.Text = stockQty.ToString("0.00");
                }
                else
                {
                    string stockQuery =
                        "select SUM(CAST(stockQty as decimal)) as totalQty from QtyManagement where storeId='" + WarehouseId + "'";
                    var dtStockQty = sqlOperation.getDataTable(stockQuery);
                    var totalQty = dtStockQty.Rows[0]["totalQty"].ToString() == ""? "0": dtStockQty.Rows[0]["totalQty"].ToString();
                    var stockQty = Convert.ToDecimal(totalQty);
        ...
        protected void btnPrintImage_Click(object sender, EventArgs e)
        {
                ...
                if (grv.RowType == DataControlRowType.DataRow)
                {
                    ...
                    if (chk.Checked)
                    {
                        ...
                        if (dtSotckBarcode.Rows.Count > 0)
                        {
                            ...
                            File.WriteAllText(Server.MapPath("BarcodeTool/BarCode.txt"), prodCode);// local /
            ...
            {
                ScriptMessage(Resources.Language.Lbl_stock_please_select_a_product, MessageType.Warning);
                return;
            }
        ...
        protected void grdStock_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            if (e.CommandName == "Edit")
            {
                ...
                Label lblProductID = (Label)row.FindControl("lblProdID");
                var storeId = ddlWarehouseList.SelectedValue.ToString();
            ...
            if (e.CommandName == "Transfer")
            {
                ...
                lblTransferAvailableQty.Text = stockUnit.getStockQtyByUnitService(lblProdId.Text, storeId);
        ...
        protected void btnStockBarcodePrintFromModal_OnClick(object sender, EventArgs e)
        {
            ...
            // Check barcode image create or not
            if (!File.Exists(Server.MapPath("/Admin/BarcodeTool/images/" + hfProductCode.Value + ".png")))
            {
                File.WriteAllText(Server.MapPath("/Admin/BarcodeTool/BarCode.txt"), hfProductCode.Value);

                Process process = Process.Start(Server.MapPath("/Admin/BarcodeTool/BarCodeGenerate.exe"));
                if (process != null) process.WaitForExit();
            }
...
//            string query =
//                @"SELECT QtyManagement.productId, QtyManagement.stockQty, StockInfo.prodCode, StockInfo.prodName, StockInfo.bPrice,StockInfo.sPrice
//                    FROM QtyManagement
//                    INNER JOIN IncrementingInfo ON " + condionalWithQty + " LEFT JOIN StockInfo ON QtyManagement.productId=StockInfo.prodID  WHERE  QtyManagement.productId= '5' AND QtyManagement.storeId='74' " + ProdCodeField.Value + "";
           ...
           var productImage = Session["pageName"] = "Image";
        ...
        protected void btnExportToCsv_OnClick(object sender, EventArgs e)
        {
            ...
            if (ddlExportCSVOption.SelectedValue == "0")
            {
                ...
                queryExportToExcel = @"Select 
                                        ...
                                        MIN(stock.manufacturerId) as manufacturerID,
                                        ((Select  (
                                       ...
                                       ((Select  MIN(stock.bPrice)*(
                                        ...
                                        FROM StockStatusInfo as stockstatus
                                        WHERE stockstatus.prodId = stock.prodID)) AS BuyTotal,
                                        ((Select  MIN(stock.dealerPrice)*(
                                                (COALESCE(SUM(CASE WHEN status = 'stock' THEN CAST(qty as decimal) END),0) +
                                                COALESCE(SUM(CASE WHEN status = 'saleReturn' THEN CAST(qty as decimal) END),0) +
                                                COALESCE(SUM(CASE WHEN status = 'stockReceive' THEN CAST(qty as decimal) END),0)) -
                                                (COALESCE(SUM(CASE WHEN status = 'sale' THEN CAST(qty as decimal) END),0) +
                                        ...
                                        FROM StockStatusInfo as stockstatus
                                        WHERE stockstatus.prodId = stock.prodID)) AS WholeSaleTotalPrice,
                                        ((Select  MIN(stock.sPrice)*(
                                                (COALESCE(SUM(CASE WHEN status = 'stock' THEN CAST(qty as decimal) END),0) +
                                                COALESCE(SUM(CASE WHEN status = 'saleReturn' THEN CAST(qty as decimal) END),0) +
                                                COALESCE(SUM(CASE WHEN status = 'stockReceive' THEN CAST(qty as decimal) END),0)) -
                                                (COALESCE(SUM(CASE WHEN status = 'sale' THEN CAST(qty as decimal) END),0) +
                                        ...
                                        FROM StockStatusInfo as stockstatus
                                        WHERE stockstatus.prodId = stock.prodID)) AS SaleTotalPrice,
                                        ((Select  (
                                                (COALESCE(SUM(CASE WHEN status = 'stock' THEN CAST(qty as decimal) END),0) +
                                                COALESCE(SUM(CASE WHEN status = 'saleReturn' THEN CAST(qty as decimal) END),0) +
                                                COALESCE(SUM(CASE WHEN status = 'stockReceive' THEN CAST(qty as decimal) END),0)) -
