...
namespace MetaPOS.Admin.InventoryBundle.Service
{
    ...
    public class Return
    {
        ...
        public Dictionary<string, string> getReturnOrDamageItemData()
        {
            ...
            var dtStock = stockModel.getProductDataByProductCode(productCode);
            if (dtStock.Rows.Count > 0)
                productId = dtStock.Rows[0]["prodId"].ToString();

            if (String.IsNullOrEmpty(productId))
                return dicReturnItem;

            var dt = returnModel.getItemDataModel(productId, storeId);
            if (dt.Rows.Count > 0)
            {
