...
namespace MetaPOS.Admin.InventoryBundle.Service
{
    ...
    public class Stock
    {
        ...
        public string saveStockStatusInfoList(dynamic data)
        {
            ...
            var entryDate = (DateTime)data["entryDate"];
        ...
        public string getStockQtyByUnitService(string prodCode, string storeId)
        {
            var prodId = "";
            var unitId = "0";
            var stockModel = new StockModel();
            var dtStock = stockModel.getProductListByProdCodeModel(prodCode);
            if (dtStock.Rows.Count > 0)
            {
                prodId = dtStock.Rows[0]["prodID"].ToString();
                unitId = dtStock.Rows[0]["unitId"].ToString();
            }
